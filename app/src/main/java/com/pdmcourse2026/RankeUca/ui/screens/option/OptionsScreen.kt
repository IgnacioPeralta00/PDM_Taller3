...
fun VotesScreen(
    ...
    val uiState by optionViewModel.uiState.collectAsState()

    LaunchedEffect(Unit) {
        optionViewModel.loadVotes()
    }
    when {
        uiState.loading -> {
        ...
        uiState.error != null -> {
            AppScaffold(
                ...
                PullToRefreshBox(
                    ...
                    onRefresh = { optionViewModel.refreshVotes() },
                    modifier = Modifier
                        .padding(contentPadding)
                        .fillMaxSize()
                ) {
                    ErrorScreen(
                        onRetryClick = { optionViewModel.refreshVotes() },
                        error = uiState.error
                    )
                }
            }
